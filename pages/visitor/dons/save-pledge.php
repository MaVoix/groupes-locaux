...
//mandatory fields
$aMandatoryFields = array("civility","name", "firstname", "email","tel","ad1","city","zipcode","country","amount");
...
if($nError==0){
    $nAmount= round( floatval(str_replace(array(" ",","),array("","."),$_POST["amount"])  ),2)  ;
    if( $nAmount<=0){
...
if($nError==0){
    $nAmount=  round( floatval(str_replace(array(" ",","),array("","."),$_POST["amount"]) ) ,2)  ;
    $nAmountMax=round($group->getAmount_target_withExpenses()-$group->getAmount_pledge()-$group->getAmount_income(),2);
...
if($nError==0){
    ...
    $pledge->setCountry($_POST["country"]);
    ...
    Mail::sendMail( $pledge->getEmail(), "Confirmation de promesse de don (" . $pledge->getReference() . ")", $sBodyMailHTML, $sBodyMailTXT, true);
